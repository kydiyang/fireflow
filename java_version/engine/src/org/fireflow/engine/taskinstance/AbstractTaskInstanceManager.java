...
//            ((TaskInstance) taskInstance).setTokenId(token.getId());
    ...
    protected abstract void beforeTaskInstanceTaskInstanceStarted(IWorkflowSession currentSession, IProcessInstance processInstance,
    ...
    public final void startTaskInstance(IWorkflowSession currentSession, IProcessInstance processInstance,
        ...
        beforeTaskInstanceTaskInstanceStarted(currentSession, processInstance, taskInstance);
        //触发事件
    ...
    protected abstract void afterTaskInstanceCompleted(IWorkflowSession currentSession, IProcessInstance processInstance,
    ...
    public final void completeTaskInstance(IWorkflowSession currentSession, IProcessInstance processInstance,
        ...
        this.afterTaskInstanceCompleted(currentSession, processInstance, taskInstance, targetActivityInstance);
...
//        System.out.println("Inside TaskInstance.complete(targetActivityInstance):: tokens.size is "+tokens.size());
    ...
    public final WorkItem createWorkItem(ITaskInstance taskInstance, String actorId) throws EngineException {
        IPersistenceService persistenceService = rtCtx.getPersistenceService();
        ...
        //调用afterWorkItemCreated
        afterWorkItemCreated(wi);
        return wi;
    }

    protected abstract void afterWorkItemCreated(IWorkItem workItem);

    public final void claimWorkItem(IWorkItem workItem) throws EngineException, KernelException {
...
//			System.out.println("WorkItem的当前状态为"+this.getState()+"，不可以执行complete操作。");
    ...
    public void completeWorkItemAndJumpTo(IWorkItem workItem, String targetActivityId, List<String> nextActorIds, boolean needClaim, String comments) throws EngineException, KernelException {
...
//        }
        ...
        if (nextActorIds != null && nextActorIds.size() > 0) {
            ...
            handler.setNeedClaim(needClaim);
            workflowSession.setCurrentDynamicAssignmentHandler(handler);
        } else {
            workflowSession.setCurrentDynamicAssignmentHandler(null);
        }
