...
//					serviceBinding.setOperation(service.getInterface().getOperation("OpenForm"));
		...
		Assert.assertEquals("/xyz.jsp", wi.getFormUrl());
		Assert.assertNotNull(wi.getActivityInstance());
		Assert.assertNotNull(wi.getCreatedTime());
		Assert.assertEquals(WorkItemAssignmentStrategy.ASSIGN_TO_ANY, wi.getAssignmentStrategy());
...
//					serviceBinding.setOperation(service.getInterface().getOperation("OpenForm"));
		...
		WorkItem wi = query.add(Restrictions.eq(WorkItemProperty.OWNER_ID, "zhangsan"))
							.add(Restrictions.eq(WorkItemProperty.ACTIVITY_INSTANCE_$_PROCESSINSTANCE_ID, actInst.getProcessInstanceId()))
							.unique();
		Assert.assertNotNull(wi);
		Assert.assertEquals(WorkItemState.INITIALIZED, wi.getState());
		Assert.assertEquals("/xyz.jsp", wi.getFormUrl());
